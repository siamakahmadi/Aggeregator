            ...
            {versions && (
              <Slider {...sliderSettings}>
                {versions.map((version) => (<div key={version.id}>{
                  version.files && version.files.map((file) => (<div>
                    <img key={file.id} src={`https://radintechco.ir/echolab/public/storage/${file.address}.${file.extension}`} alt={file.original_name}/>
                  </div>))
                }</div>))}
              </Slider>
            )}
