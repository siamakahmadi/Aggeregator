...
export default function Page({params}pro) {
  ...
  const postsLists = posts.message === "Post fetched" ?
    <>
      {posts.data.map(item => (
          <Card key={item.id} title={item.title} src="https://radintechco.ir/echolab/public/storage/%7Bd8e9107e84b3ca04507e8b075db824b0%7D" route={`users/content/${item.id}`} />
      ))}
    </> : <div>wating...</div>
