...
export default function Page({params}) {
  ...
  const postsLists = posts.message === "Post fetched" ?
    <>
      {posts.data.map(item => (
          <Card key={item.id} title={item.title} src="ssdsdsdsd" route={`users/content/${item.id}`} />
      ))}
    </> : <div>wating...</div>
...
const bookmarksList = bookmarks.message === "Post fetched" ?
    <>
      {bookmarks.data.map(item => (
          ...
          <Card key={item.id} src="ssdsdsdsd" title={item.title} route={`users/content/${item.id}`} />
      ))}
    </> : <div>wating...</div>
