...
const bookmarksList = bookmarks.message === "Post fetched" ?
    <>
      {bookmarks.data.map(item => (<>{console.log(item.versions.files.address)}</>
        console.log(item.versions.files.address)
          <Card key={item.id} src={item.versions.files.address} title={item.title} route={`users/content/${item.id}`} />
      ))}
    </> : <div>wating...</div>
