...
export default function Index(props) {
    ...
    return (
        <div href={`content/${props.route}`} className={Styles.container} >
            <div key={props.key} className={Styles.cardImage}>
                ...
                <div className={theme === 'dark' ? Styles.imageToolbarDark : Styles.imageToolbarLight}>
                    <div className={Styles.add}>
                        {!isBookmark ? <div onClick={() => { !isBookmark ? setIsBookmark(true) : setIsBookmark(false) }}> <Plus /></div> : <div className={Styles.isBookmark} onClick={() => { !isBookmark ? setIsBookmark(true) : setIsBookmark(false) }}> <TickIcon /></div>}
                    </div>
