...
export default function Home() {
  ...
  console.log(loginDone.data.token)

  const token  loginDone.data.token

  async function loginUser() {
    cookies.set('isloggin', {
      isLoggin:`true`,
      userToken:`${loginDone.data.token}`,
      userEmail:`${loginDone.data.user_info.email}`,
      userId:`${loginDone.data.user_info.id}`
    }, {
      path: '/',
    ...
    https.post(`admin/login?email=${formData.email}&password=${formData.password}`)
      .then(
        Response => {
          ...
          loginUser(Response.data.token,Response.data.email,Response.data.id)
        }
      ).catch(error => {
        console.log(error)
      })
