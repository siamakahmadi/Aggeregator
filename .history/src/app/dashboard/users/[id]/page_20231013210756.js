...
export default function Page({params}) {
  ...
  useEffect(() => {
    https.get('admin/user/action/20').then(
      Response => {
          setUsers(Response.data.data.data),
          toast('Users successfully susspended');
      }
    ).catch(error => {
      toast(`we cant sesspended Users ${error}`);
    })
  }, [])
  ...
  const tableItems = users ?
    users.map(item => (
      <TableItem key={item.id} name={item.name} email={item.email} date="date"  status={item.is_premium === 0 ? 'Free' : 'Premium'} type={item.is_premium === 0 ? 'Draft' : ''} link={'list'} />
    )) : <><div>Loading</div></>
