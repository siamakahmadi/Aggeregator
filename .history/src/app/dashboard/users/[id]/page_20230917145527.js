...
export default function Page({params}) {
  ...
  useEffect(() => {
    if(params.id === 'list'){
      ...
      https.get(`admin/user/index?search=${params.id}&paginate=20`).then(
        Response => {
  ...
  const tableItems = users ?
    users.map(item => (
      <TableItem key={item.id} name={item.name} email={item.email} date="date" suspand status={item.is_premium === 0 ? 'Free' : 'Premium'} type={item.is_premium === 0 ? 'Draft' : ''} link={'list'} />
    )) : <><div>Loading</div></>
