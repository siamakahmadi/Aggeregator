...
export default function Page({params}) {
  ...
  const tableItems = users ?
    users.map(item => (
      <TableItem key={item.id} name={item.name} email={item.email} date="date"  status={item.is_premium === 0 ? 'Free' : 'Premium'} type={item.is_premium === 0 ? 'Draft' : ''} handleSusspend={handleSusspend} />
    )) : <><div>Loading</div></>
