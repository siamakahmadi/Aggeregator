...
export default function Page({params}) {
  ...
  useEffect(() => {
    if(params.id === 'list'){
      ...
      https.get(`admin/user/detail/${params.id}`).then(
        ...
        setInterval(()=>{router.push('/dashboard/users/list')},3000)
  ...
  const tableItems = users ?
    users.map(item => (
      <TableItem key={item.id} link={item.id} name={item.name}  onClick={()=>handleSusspend(item.id)} email={item.email} date="date" id={item.id} status={item.is_premium === 0 ? 'Free' : 'Premium'} type={item.is_premium === 0 ? 'Draft' : ''} />
    )) : <><div>Loading</div></>
      ...
      <div className={Styles.table}>
        <div className={Styles.tableHeader}>
          <div className={Styles.tableTitle}>
            <span>List Of Users</span><StatusLabel title={`${userTotal} Users`} />
          </div>
        ...
        <div className={Styles.tableBody}>
          { params.id === 'list' ? <></> :
          <Modal title={'Title'} onClickHandle={() => router.back()}>
