...
export default function Page({params}) {
  ...
  const tableItems = users ?
    users.map(item => (
      <TableItem key={item.id} handleClick={console.log()} name={item.name} email={item.email} date="date" id={item.id} status={item.is_premium === 0 ? 'Free' : 'Premium'} type={item.is_premium === 0 ? 'Draft' : ''} />
    )) : <><div>Loading</div></>
