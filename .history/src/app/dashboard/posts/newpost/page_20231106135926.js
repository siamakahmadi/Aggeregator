...
export default function NewPost() {
  ...
  useEffect(() => {
    https
      ...
      .then((Response) => {
        setTypeFace(Response.data.data);
      })
      .catch((error) => {
        toast(`we cant featched categories`);
      });
  ...
  if (Array.isArray(category)) {
    const categoryItems = category.map((item) => item.name);
    // Now you can safely use categoryItems.
  } else {
    // Handle the case when category is not an array.
