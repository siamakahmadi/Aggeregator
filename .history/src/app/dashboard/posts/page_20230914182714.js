...
export default function Page() {
  ...
  const errorNotify = () => toast("Wow so easy!");
  const successNotify = () => toast("action null");
      ...
      {posts.data.data.map(item => (
        <div key={item.id} className={styles.row}>
          ...
          <div className={styles.rightSide}>
            ...
            <div className={styles.Functional} onClick={() => deletePost(item.id)} >
  ...
  useEffect(() => {
    https.get('admin/browse/post').then(Response => {
      ...
      <ToastContainer>Wow so easy!</ToastContainer>
    }).catch(error => {
      console.log(error)
    })
  }, [])
