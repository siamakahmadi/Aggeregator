...
export default function Page() {
      ...
      {posts.data.data.map(item => (
        <div key={item.id} className={styles.row}>
          ...
          <div className={styles.rightSide}>
            ...
            <a className={styles.Functional} >
              {item.status === 'Published' ? <p onClick={() => switchDraft(item.id, 'Published')}>Draft</p> : <p onClick={() => switchDraft(item.id , 'Draft')}>Publish</p>}
            </a>
  ...
  function switchDraft(id,Status) {
    ...
    https.get(`admin/post/action/${id}?action=publisg`).then(
      Response => {
        console.log(Response.status)
      }
    ).catch(error => {
