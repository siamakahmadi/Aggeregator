...
export default function Page() {
  ...
  const [isPublish,setIsPublish] = (true)
        ...
        <div key={item.id} className={styles.row}>
          ...
          <div className={styles.rightSide}>
            ...
            <div className={styles.status}>
              <StatuLabel title={isPublish ? 'Published' : 'Draft'} type={isPublish ? 'Published' : 'Draft'} />
            </div>
            <a className={styles.Functional} >
              <p>View</p>
            </a>
            <a className={styles.Functional} >
              {item.status === 'Published' ? <p onClick={() => switchDraft(item.id, 'draft')}>Draft</p> : <p onClick={() => switchDraft(item.id, 'publish')}>Publish</p>}
            </a>
  ...
  async function switchDraft(id, action) {
    await https.get(`admin/post/action/${id}?action=${action}`).then(
      action === 'Published' ?  setIsPublish(true) : setIsPublish(false) 
