...
export default function Page() {
      ...
      {posts.data.data.map(item => (
        <div key={item.id} className={styles.row}>
          ...
          <div className={styles.rightSide}>
            ...
            <a className={styles.Functional} >
              {item.status === 'Published' ? <p onClick={() => action.switchDraft(item.id, 'draft')}>Draft</p> : <p onClick={() => action.switchDraft(item.id, 'publish')}>Publish</p>}
            </a>
  ...
  useEffect(() => {
    ...
    const action = switchDraft(id, action){
