...
export default function Page() {
      ...
      {posts.data.data.map(item => (
        <div key={item.id} className={styles.row}>
          ...
          <div className={styles.rightSide}>
            ...
            <a className={styles.Functional} >
              {item.status === 'Published' ? <p onClick={() => switchDraft(item.id)}>Draft</p> : <p onClick={() => switchDraft(item.id)}>Publish</p>}
            </a>
  ...
  function switchDraft(id) {
    posts.data.data.status === 'Published' ?
    
    posts.data.data.map(item => console.log(item.status))
    :
    posts.data.data[id](item => 
      // https.get(`admin/post/action/${id}?action=Published`).then(
      //   Response => {
      //     console.log(Response.status)
      //   }
      // ).catch(error => {
    ...
    // console.log(posts.data.data.status)    
    // https.get(`admin/post/action/${id}?action=draft`).then(
    //   Response => {
    //     console.log(Response.status)
    //   }
    // ).catch(error => {
