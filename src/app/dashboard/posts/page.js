  ...
  export default function Page() {
        ...
        {posts.data.data.map(item => (
          <div key={item.id} className={styles.row}>
            ...
            <div className={styles.rightSide}>
              ...
              <a className={styles.Functional} onClick={()=>router.push(`/dashboard/posts/view/${item.id}`)}>
                <p className={styles.view}>View</p>
              </a>
              <a className={styles.Functional} >
                {item.status === 'Published' ? <p className={styles.draft} onClick={() => switchDraft(item.id, 'draft')}>Draft</p> : <p className={styles.publish} onClick={() => switchDraft(item.id, 'publish')}>Publish</p>}
              </a>
              <div className={styles.Functional} onClick={() => deletePost(item.id, item.title)} >
    ...
    function switchDraft(id, action) {
      https.get(`admin/post/action/${id}?action=${action}`).then(
        Response => {
